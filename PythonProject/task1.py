...
cv2.createTrackbar("h min","TrackBars",151,180,empty)
cv2.createTrackbar("h max","TrackBars",178,180,empty)
cv2.createTrackbar("s min","TrackBars",20,255,empty)
cv2.createTrackbar("s max","TrackBars",205,255,empty)
cv2.createTrackbar("v min","TrackBars",73,255,empty)
cv2.createTrackbar("v max","TrackBars",255,255,empty)

# 无限循环，实时读取滑动条的值并更新图像，从而确定 HSV 范围
while True:
    h_min = cv2.getTrackbarPos("h min", "TrackBars")
    img_bgr = cv2.imread("resources/bed_pic.png")
